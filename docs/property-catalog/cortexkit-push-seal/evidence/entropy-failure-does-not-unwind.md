...
- Code trail: public `seal` explicitly supplies `hpke::rand_core::UnwrapErr(getrandom::SysRng)` to `seal_with_rng` and documents the panic. The crate declares caret requirements `getrandom = "0.4"` and `hpke = "0.14"`; the observed resolution is `getrandom 0.4.3` and `hpke 0.14.0`.
