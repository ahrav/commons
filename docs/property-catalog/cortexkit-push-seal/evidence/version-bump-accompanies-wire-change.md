...
- Existing evidence: synthetic tests reject a represented fixture change with an unchanged version, accept the same change with a version bump, accept initial fixture bootstrap, ignore formatting/provenance-only or unrelated changes, and fail loudly when a fixture lacks any represented contract field. The actual gate skips an unreadable base revision (no comparable base exists) and fails on an unreadable head revision.
