...
- Existing evidence: a direct correct open with `[VERSION]` succeeds; direct opens with empty, wrong one-byte, and extra-byte AAD return exact `HpkeError::OpenError` against the same envelope and empty `info`.
