...
- **Primary evidence:** `open_sqlite` acquires epoch but does not claim the database fence (`cortexkit-store/src/lib.rs:248-286`); claim is lazy inside `with_conn_fenced` (`:173-215`).
- **Existing evidence:** synthetic handover test claims epoch 2 before stale epoch 1 attempts a write (`:631-670`); equal epoch is permitted (`:672-691`).
