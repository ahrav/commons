...
- **Scope:** sqlite/file backend only. PostgreSQL uses a server-side session advisory lock across processes and machines, not a filesystem lock (`crates/cortexkit-store-postgres/src/lib.rs:7-9,201-220`).
...
- **Open-question log:** no sqlite deployment inventory was supplied. `cortexkit-store` co-locates the file lease and sqlite database (`crates/cortexkit-store/src/lib.rs:225-242`), so sqlite database placement determines this property. PostgreSQL's separate session-lock scope requires its own server/session-loss evidence, not a mount-capability check.
