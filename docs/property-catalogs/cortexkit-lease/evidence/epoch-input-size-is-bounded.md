...
- **Primary evidence:** `read_epoch` uses `Read::take(21)`, preallocates 21 bytes, and rejects lengths above the 20-byte decimal maximum (`crates/cortexkit-lease/src/lib.rs:342-364`).
- **Existing evidence:** `invalid_epoch_states_fail_closed` exercises a 21-byte file through exclusive and shared acquisition (`crates/cortexkit-lease/src/lib.rs:544-586`).
