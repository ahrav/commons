...
- **Primary evidence:** contract at `crates/cortexkit-lease/src/lib.rs:2-6`; `read_epoch`, `bump_epoch_above`, and `persist_epoch` provide bounded parsing, checked increment above persisted state and an optional resource floor, and fixed-width update.
