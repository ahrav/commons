...
- **Primary evidence:** `read_epoch` accepts only 1-20 ASCII digits in `u64` range (`src/lib.rs:342-364`).
...
- **Existing evidence:** `invalid_epoch_states_fail_closed` exercises both acquisition modes for parser-invalid states and preserves bytes (`src/lib.rs:544-586`). `maximum_epoch_is_readable_but_exhausted` separates valid shared reads from exclusive exhaustion (`src/lib.rs:589-612`).
