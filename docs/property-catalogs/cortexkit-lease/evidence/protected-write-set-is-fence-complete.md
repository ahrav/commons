...
- **Primary evidence:** SQLite exposes `with_conn`, `with_conn_fenced`, and `migrate` (`cortexkit-store/src/lib.rs:145-229`); PostgreSQL exposes `with_client` and `migrate` without comparing epoch (`cortexkit-store-postgres/src/lib.rs:93-109`).
