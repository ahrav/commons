...
- **Primary evidence:** `acquire_then_second_holder_is_rejected` is same-process and sequential (`crates/cortexkit-lease/src/lib.rs:490-506`); `shared_lease_across_processes_blocks_exclusive` is cross-process but shared-versus-exclusive (`crates/cortexkit-lease/src/lib.rs:630-691`).
