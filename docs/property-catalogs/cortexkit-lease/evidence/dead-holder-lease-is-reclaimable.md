...
- **Primary evidence:** the file-backend contract says the kernel releases the advisory lock on process death (`crates/cortexkit-lease/src/lib.rs:4`). PostgreSQL instead holds a session advisory lock, which the server releases when the connection drops (`crates/cortexkit-store-postgres/src/lib.rs:7-9,56-63`); normal drop also attempts `pg_advisory_unlock` before closing the connection (`crates/cortexkit-store-postgres/src/lib.rs:91-98`).
- **Existing evidence:** `shared_lease_across_processes_blocks_exclusive` lets the child exit normally after a sleep; every in-process file release runs `Drop`. PostgreSQL tests reopen only after normal `PostgresStore` drop (`crates/cortexkit-store-postgres/src/lib.rs:305-341`).
