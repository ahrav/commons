...
- **Existing evidence:** `shared_lease_across_processes_blocks_exclusive` lets the child exit normally after a sleep; every in-process file release runs `Drop`. `superseded_writer_is_rejected_after_reopen` shows PostgreSQL reopening only after a normal drop (`crates/cortexkit-store-postgres/src/lib.rs:1019-1051`).
