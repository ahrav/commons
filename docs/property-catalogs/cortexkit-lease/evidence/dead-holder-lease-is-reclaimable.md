...
- **Primary evidence:** the file-backend contract says the kernel releases the advisory lock on process death (`crates/cortexkit-lease/src/lib.rs:7-10`). PostgreSQL instead holds a session advisory lock, which the server releases when the connection drops (`crates/cortexkit-store-postgres/src/lib.rs:13-22,77-84`); normal drop also attempts `pg_advisory_unlock` before closing the connection (`crates/cortexkit-store-postgres/src/lib.rs:112-119`).
- **Existing evidence:** the file-backend cross-process test lets the child exit normally after a sleep (`crates/cortexkit-lease/src/lib.rs:630-691`); every in-process file release runs `Drop`. PostgreSQL tests reopen only after normal `PostgresStore` drop (`crates/cortexkit-store-postgres/src/lib.rs:326-362`).
