...
- **Primary evidence:** `LeaseHandle::epoch` has one return type for both modes; `LeaseStore::acquire` and `LeaseStore::acquire_shared` both return `Box<dyn LeaseHandle>`, while the shared-method docs restrict its epoch to observation.
