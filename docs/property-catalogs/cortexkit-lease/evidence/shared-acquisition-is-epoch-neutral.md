...
- **Primary evidence:** `open_lease_file` initializes canonical zero before no-clobber publication, and `LeaseStore::acquire_shared` uses only `try_lock_shared` plus `read_epoch` after open.
