...
- **Primary evidence:** headline contract at `crates/cortexkit-lease/src/lib.rs:2-6`; `FileLeaseStore::lease_path` derives one per-key path and `LeaseStore::acquire` calls `File::try_lock`, mapping `TryLockError::WouldBlock` to `Held` and other lock errors to `Io`. PostgreSQL derives one advisory key and treats a false `pg_try_advisory_lock` result as `Held` (`crates/cortexkit-store-postgres/src/lib.rs:54-67,242-252`).
