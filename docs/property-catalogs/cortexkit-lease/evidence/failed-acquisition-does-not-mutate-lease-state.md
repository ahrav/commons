...
- **Primary evidence:** file-backed exclusive and shared acquisition call `open_lease_file`, including descriptor hardening, before attempting the lock. PostgreSQL tries its session advisory lock before creating infrastructure tables or bumping the epoch (`crates/cortexkit-store-postgres/src/lib.rs:116-142`).
