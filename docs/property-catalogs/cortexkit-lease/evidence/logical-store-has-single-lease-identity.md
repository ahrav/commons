...
- **Primary evidence:** lease key excludes database path (`cortexkit-store/src/lib.rs:69-75`); root is the database parent (`:245-260`).
- **Existing evidence:** `distinct_databases_do_not_falsely_contend` (`:493-501`) uses different parent directories, so it cannot expose sibling-file aliasing.
