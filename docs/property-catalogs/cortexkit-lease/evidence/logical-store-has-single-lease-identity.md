...
- **Primary evidence:** lease key excludes database path (`cortexkit-store/src/lib.rs:80-88`); root is the database parent (`:240-262`).
- **Existing evidence:** `distinct_databases_do_not_falsely_contend` (`:495-504`) uses different parent directories, so it cannot expose sibling-file aliasing.
