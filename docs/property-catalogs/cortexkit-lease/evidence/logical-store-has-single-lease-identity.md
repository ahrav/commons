...
- **Primary evidence:** lease key excludes database path (`cortexkit-store/src/lib.rs:80-86`); root is the database parent (`:570-585`).
- **Existing evidence:** `distinct_databases_do_not_falsely_contend` (`:1067-1076`) uses different parent directories, so it cannot expose sibling-file aliasing.
