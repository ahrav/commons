...
- **Primary evidence:** lease key excludes database path (`cortexkit-store/src/lib.rs:69-78`); root is the database parent (`:225-242`).
- **Existing evidence:** `distinct_databases_do_not_falsely_contend` (`:625-633`) uses different parent directories, so it cannot expose sibling-file aliasing.
