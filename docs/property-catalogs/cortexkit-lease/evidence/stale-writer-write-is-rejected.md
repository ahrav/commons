...
- **Existing evidence:** `superseded_writer_is_fenced_out_after_handover` (`crates/cortexkit-store/src/lib.rs:1492-1531`) uses synthetic stores with independently supplied epochs. `equal_epoch_writer_is_not_fenced` (`:1566-1587`) proves equal epochs remain authorized on the write path, and `open_claim_rejects_an_epoch_the_database_already_stores` (`:850-887`) proves the open path refuses them. PostgreSQL separately checks callback rollback, equal-epoch repeats, and synthetic stale rejection (`crates/cortexkit-store-postgres/src/lib.rs:918-1008`). Both backends reject stale migrations before schema SQL.
