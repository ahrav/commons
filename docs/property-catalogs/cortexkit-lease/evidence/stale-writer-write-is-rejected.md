...
- **Existing evidence:** `superseded_writer_is_fenced_out_after_handover` (`crates/cortexkit-store/src/lib.rs:1632-1670`) uses synthetic stores with independently supplied epochs. `equal_epoch_writer_is_not_fenced` (`:1706-1726`) proves equal epochs remain authorized on the write path, and `open_claim_rejects_an_epoch_the_database_already_stores` (`:941-977`) proves the open path refuses them. PostgreSQL separately checks callback rollback in `fenced_callback_error_rolls_back_rows` (`crates/cortexkit-store-postgres/src/lib.rs:1086-1119`) through synthetic stale rejection in `superseded_writer_cannot_migrate` (`crates/cortexkit-store-postgres/src/lib.rs:1178-1213`). Both backends reject stale migrations before schema SQL.
