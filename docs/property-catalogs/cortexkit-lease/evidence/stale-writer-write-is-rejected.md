...
- **Existing evidence:** `superseded_writer_is_fenced_out_after_handover` (`crates/cortexkit-store/src/lib.rs:1578-1617`) uses synthetic stores with independently supplied epochs. `equal_epoch_writer_is_not_fenced` (`:1652-1673`) proves equal epochs remain authorized on the write path, and `open_claim_rejects_an_epoch_the_database_already_stores` (`:921-958`) proves the open path refuses them. PostgreSQL separately checks callback rollback, equal-epoch repeats, and synthetic stale rejection (`crates/cortexkit-store-postgres/src/lib.rs:961-1051`). Both backends reject stale migrations before schema SQL.
