...
- **Primary evidence:** the `LeaseHandle` contract ties ownership to handle lifetime; `FileLeaseHandle::drop` calls standard-library `File::unlock`. Acquisition and read failures also call `File::unlock` before returning. Reacquisition tests cover normal drops.
