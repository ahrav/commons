...
- **Primary evidence:** exclusive acquisition unlocks on persistence error (`FileLeaseStore::acquire_above`); `persist_epoch` never truncates and writes canonical digits from most to least significant (`crates/cortexkit-lease/src/lib.rs:242-276,443-451`).
