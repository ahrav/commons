...
- **Primary evidence:** exclusive acquisition unlocks on persistence error (`FileLeaseStore::acquire_above`); `persist_epoch` never truncates and writes canonical digits from most to least significant (`crates/cortexkit-lease/src/lib.rs:260-286,444-452`).
...
- **Existing evidence:** `interrupted_persist_never_leaves_a_lower_parseable_epoch` injects short ordered writes through production `persist_epoch` for legacy-width and canonical-width prior states, parses through production `read_epoch`, and proves any parseable aftermath is not lower; the asserted count of parseable aftermaths keeps the canonical digit-over-digit splice covered rather than skipped (`crates/cortexkit-lease/src/lib.rs:730-842`).
