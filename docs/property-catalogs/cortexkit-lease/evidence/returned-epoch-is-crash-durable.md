...
- **Contradictory code evidence:** no `sync_data`, `sync_all`, or directory sync; `flush` at `crates/cortexkit-lease/src/lib.rs:450` is not a stable-storage barrier for `File`.
