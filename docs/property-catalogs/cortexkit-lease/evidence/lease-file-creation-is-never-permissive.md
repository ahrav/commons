...
- **Primary evidence:** after an existing-path open returns `NotFound`, `open_lease_file` creates a same-directory `NamedTempFile`, initializes it, and publishes the same open inode with `persist_noclobber` (`src/lib.rs:106-143`). `tempfile`'s private-file creation contract is part of this property.
