...
- **Existing check:** `open_claims_fence_before_return`, `cortexkit-store/src/lib.rs:902-915`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:749-786`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
- **Existing check:** backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, both autocommit maintenance paths, that a panicking read leaves later fenced writes authorized (`cortexkit-store/src/lib.rs:1189-1210`), that protected transactions re-pin `synchronous=FULL` (`:1212-1277`), that a transaction-ending callback or migration is denied on SQLite and rejected on PostgreSQL (`:1162-1219`; `cortexkit-store-postgres/src/lib.rs:625-668`), that a read callback ending its transaction is rejected (`:670-730`), that a negative epoch fails closed (`:732-763`), that a regressed positive epoch fails closed for the holder and for a superseded writer above it (`:765-808`), and that a callback cannot damage the lease row it was fenced against (`:810-855`), and that an epoch which did not advance past the stored row, or which the committed row does not carry, fails the open (`:857-875,877-902`); SQLite refuses the same damage to its fence row before it executes (`cortexkit-store/src/lib.rs:1394-1469`). The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
