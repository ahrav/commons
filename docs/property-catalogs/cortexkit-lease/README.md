...
- Revision: `9e871ce` plus the U8 working-tree changes
- Date: 2026-09-01
...
- **Type:** safety
- **Status:** active
- **Exercised:** yes for existing empty content through ordinary acquisition, and for non-decimal UTF-8, trailing whitespace, invalid UTF-8, oversized input, and 20-digit `u64` overflow through ordinary and floor-based acquisition.
...
- **Existing check:** `superseded_writer_is_fenced_out_after_handover`, `cortexkit-store/src/lib.rs:1632-1670`, and `superseded_writer_is_rejected_after_reopen`, `cortexkit-store-postgres/src/lib.rs:1144-1175`; both use synthetic stale stores and remain **unaudited**.
...
- **Confidence:** high on derivation facts (`cortexkit-store/src/lib.rs:80-86,570-622`); unknown whether descriptor authority prevents these combinations in deployment.
- **Existing check:** `distinct_databases_do_not_falsely_contend`, `cortexkit-store/src/lib.rs:1057-1065`, uses different parent directories and does not exercise sibling files; status **unaudited**.
...
- **Confidence:** high that every returned store has claimed a strictly greater epoch (`cortexkit-store/src/lib.rs:570-622,689-703`); the stronger acquisition-instant guarantee remains unproved.
- **Existing check:** `open_claims_fence_before_return`, `cortexkit-store/src/lib.rs:922-934`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:941-977`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
