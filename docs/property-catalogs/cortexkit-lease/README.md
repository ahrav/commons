...
- **Existing check:** `superseded_writer_is_fenced_out_after_handover`, `cortexkit-store/src/lib.rs:1638-1677`, and `superseded_writer_is_rejected_after_reopen`, `cortexkit-store-postgres/src/lib.rs:1092-1124`; both use synthetic stale stores and remain **unaudited**.
...
- **Existing check:** `distinct_databases_do_not_falsely_contend`, `cortexkit-store/src/lib.rs:1066-1075`, uses different parent directories and does not exercise sibling files; status **unaudited**.
...
- **Existing check:** `open_claims_fence_before_return`, `cortexkit-store/src/lib.rs:931-944`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:950-987`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
- **Existing check:** backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, both autocommit maintenance paths, that a panicking read leaves later fenced writes authorized (`cortexkit-store/src/lib.rs:1189-1210`), that protected transactions re-pin `synchronous=FULL` (`:1212-1277`), that `a_callback_cannot_end_the_fence_checked_transaction` denies the escape on SQLite (`:1364-1421`) while `a_callback_that_ends_the_transaction_is_rejected` reports it on PostgreSQL (`cortexkit-store-postgres/src/lib.rs:660-703`), that a read callback ending its transaction is rejected (`:670-730`), that a negative epoch fails closed (`:732-763`), that a regressed positive epoch fails closed for the holder and for a superseded writer above it (`:765-808`), that a callback cannot release the advisory lease it holds, and that a callback cannot damage the lease row it was fenced against (`:810-855`), and that an epoch which did not advance past the stored row, or which the committed row does not carry, fails the open (`:857-875,877-902`); SQLite refuses the same damage to its fence row before it executes (`cortexkit-store/src/lib.rs:1394-1469`). The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
