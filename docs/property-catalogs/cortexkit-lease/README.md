...
- **Existing check:** `superseded_writer_is_fenced_out_after_handover`, `cortexkit-store/src/lib.rs:1446-1485`, and `superseded_writer_is_rejected_after_reopen`, `cortexkit-store-postgres/src/lib.rs:932-964`; both use synthetic stale stores and remain **unaudited**.
...
- **Existing check:** `distinct_databases_do_not_falsely_contend`, `cortexkit-store/src/lib.rs:938-947`, uses different parent directories and does not exercise sibling files; status **unaudited**.
...
- **Existing check:** `open_claims_fence_before_return`, `cortexkit-store/src/lib.rs:803-816`, observes the claim before domain setup, and `open_claim_rejects_an_epoch_the_database_already_stores`, `:749-786`, pins the strict-advance rule at the helper rather than through two racing opens; status **unaudited**.
...
- **Existing check:** backend tests cover fenced callbacks, fenced migrations, SQLite and PostgreSQL read-only rejection, both autocommit maintenance paths, that a panicking read leaves later fenced writes authorized (`cortexkit-store/src/lib.rs:1090-1111`), that protected transactions re-pin `synchronous=FULL` (`:1113-1178`), that a transaction-ending callback or migration is denied on SQLite and rejected on PostgreSQL (`:1162-1219`; `cortexkit-store-postgres/src/lib.rs:585-628`), that a read callback ending its transaction is rejected (`:630-690`), that a negative epoch fails closed (`:692-723`), that a regressed positive epoch fails closed for the holder and for a superseded writer above it (`:725-768`), and that a callback cannot damage the lease row it was fenced against (`:770-815`); SQLite refuses the same damage to its fence row before it executes (`cortexkit-store/src/lib.rs:1295-1337`). The [durable consumer inventory](durable-consumer-inventory.md) records source receipts; no source-level completeness gate exists.
