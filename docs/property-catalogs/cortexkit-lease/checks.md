...
|---|---|---|---|
| `reopening_a_permissive_store_protects_the_database_and_its_wal` | `cortexkit-store/src/lib.rs:809-864` | Database and WAL are `0600` on reopen. | unaudited |
| `open_claims_fence_before_return` | `cortexkit-store/src/lib.rs:902-915` | Open stamps the lease epoch before exposing the store. | unaudited |
| `open_claim_rejects_an_epoch_the_database_already_stores` | `cortexkit-store/src/lib.rs:921-958` | The open claim rejects an epoch equal to the stored fence; `claim_fence` still authorizes it. | unaudited |
| `migrations_seed_once_across_reopen` | `cortexkit-store/src/lib.rs:960-982` | Migrations and seeds run once; clean reopen issues a greater epoch. | unaudited |
| `database_epoch_survives_repeated_lease_sidecar_loss` | `cortexkit-store/src/lib.rs:984-1011` | Two repeated sidecar losses each issue an epoch above the database fence. | unaudited |
| `second_live_writer_is_rejected` | `cortexkit-store/src/lib.rs:1025-1035` | Second same-process store open is rejected as a lease error. | unaudited |
| `distinct_databases_do_not_falsely_contend` | `cortexkit-store/src/lib.rs:1037-1046` | Distinct database paths coexist. | unaudited |
| `unfenced_connection_rejects_writes` | `cortexkit-store/src/lib.rs:1152-1176` | `with_conn` rejects a write with `SQLITE_READONLY`, leaves no row, and still permits a later fenced write. | unaudited |
| `open_pins_full_synchronous` | `cortexkit-store/src/lib.rs:1178-1187` | Open pins `synchronous=FULL`, so a committed fence epoch survives power loss in WAL mode. | unaudited |
| `a_panicking_read_does_not_strand_the_connection_read_only` | `cortexkit-store/src/lib.rs:1189-1210` | A panicking `with_conn` callback still clears `query_only`, so later fenced writes and maintenance remain authorized. | unaudited |
| `a_read_callback_cannot_lower_fence_durability` | `cortexkit-store/src/lib.rs:1212-1277` | A read callback is denied lowering `synchronous`, and a fenced write and a migration re-pin `FULL` and a WAL journal after the maintenance path changes them. | unaudited |
| `a_read_callback_cannot_clear_the_read_only_guard` | `cortexkit-store/src/lib.rs:1279-1333` | A read callback is denied every pragma write, in any letter case, and writes nothing. | unaudited |
| `a_callback_cannot_damage_the_fence_row_it_is_checked_against` | `cortexkit-store/src/lib.rs:1394-1469` | A fenced callback or migration that lowers, deletes, or forges the fence and version rows is denied, and the row keeps its epoch. | unaudited |
| `a_callback_cannot_end_the_fence_checked_transaction` | `cortexkit-store/src/lib.rs:1335-1392` | `COMMIT`, `ROLLBACK`, `SAVEPOINT`, and `BEGIN` are denied inside a fenced callback and a migration, and nothing commits or is created unfenced. | unaudited |
| `maintenance_runs_through_the_unfenced_path` | `cortexkit-store/src/lib.rs:1471-1485` | `VACUUM` fails the read-only guard and succeeds through `with_conn_unfenced`. | unaudited |
| `fenced_write_rolls_back_on_error` | `cortexkit-store/src/lib.rs:1487-1521` | Callback failure rolls back both domain mutation and a newer fence claim. | unaudited |
| `legacy_database_without_fence_table_uses_zero_floor` | `cortexkit-store/src/lib.rs:1523-1546` | A pre-fence-table database opens at floor zero and receives epoch 1. | unaudited |
| `legacy_negative_database_fence_fails_closed` | `cortexkit-store/src/lib.rs:1548-1576` | A pre-constraint negative fence is rejected and remains unchanged. | unaudited |
| `superseded_writer_is_fenced_out_after_handover` | `cortexkit-store/src/lib.rs:1578-1617` | Synthetic epoch-1 writer cannot overwrite epoch-2 state. | unaudited |
| `superseded_writer_cannot_migrate` | `cortexkit-store/src/lib.rs:1619-1650` | Synthetic stale migration is fenced before its schema SQL executes. | unaudited |
| `equal_epoch_writer_is_not_fenced` | `cortexkit-store/src/lib.rs:1652-1673` | Equal epoch can continue writing. | unaudited |
| `epoch_above_sqlite_integer_range_fails` | `cortexkit-store/src/lib.rs:1675-1691` | Epochs above SQLite's signed integer range fail instead of wrapping. | unaudited |
